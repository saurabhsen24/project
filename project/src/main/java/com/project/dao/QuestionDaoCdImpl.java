...
@Repository
public class QuestionDaoCdImpl implements QuestionDaoCd{
	
	@Autowired
	private JdbcTemplate jdbcTemplate;

	private static final String INSERT_QUERY = "INSERT INTO QUESTIONCD(que,opt1,opt2,opt3,opt4,ans) values(?,?,?,?,?,?)";
	private static final String SELECT_ALL = "SELECT * from QUESTIONCD";
	private static final String UPDATE_QUERY = "UPDATE QUESTIONCD SET qid = ?,que = ?,opt1 = ?,opt2 = ?,opt3 = ?,opt4 = ?,ans = ? where qid = ?"; 
	private static final String DELETE_QUERY = "DELETE from QUESTIONCD where qid = ?";	
